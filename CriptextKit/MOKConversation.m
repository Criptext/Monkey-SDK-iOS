...
- (id)initWithUserAnonymous:(NSString *)userId_{
	if (self = [super init]) {
        ...
        MOKUser *userAnonymous= [[MOKUser alloc]initWithUserId:[[NSUUID UUID] UUIDString] andParams:nil];
...
-(int)getTotalMessagesWithoutRead{
    ...
    for (int i=0; i<[mensajes count]; i++) {
        MOKMessage * msj = [mensajes objectAtIndex:i];
        if(!msj.readByUser)
            total++;
    }
    return total;
...
-(MOKMessage *)lastMessageWithoutRead{
    for(int i=[mensajes count]-1;i>=0;i--){
        MOKMessage *message=[mensajes objectAtIndex:i];
