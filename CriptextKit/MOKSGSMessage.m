...
+ (id)message {
	return [[[MOKSGSMessage alloc] init] autorelease];
}

+ (id)messageWithData:(NSData *)theData {
	return [[[MOKSGSMessage alloc] initWithData:theData] autorelease];
}

+ (id)sessionMessage {
	MOKSGSMessage *msg = [[[MOKSGSMessage alloc] init] autorelease];
