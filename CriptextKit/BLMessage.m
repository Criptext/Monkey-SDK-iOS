...
- (id)initWithArgs:(NSDictionary*)dictionary{
	...
	if (self = [super init]) {
		...
		if([dictionary objectForKey:@"uid_to"]!=nil){
            ...
            self.userIdTo = [SessionManager sharedInstance].userId;
...
- (id)initWithMyMessage:(NSString*)_messageText userTo:(NSString *)_userId {
	if (self = [super init]) {
		...
		self.userIdFrom = [SessionManager sharedInstance].userId;
		//self.messageId = 0;
		self.readByUser = NO;
		self.type = blMessageDefault;
		self.isSending = NO;
...
- (id)initWithMyMessageAnonymous:(NSString*)_messageText userTo:(NSString *)_userId {
	if (self = [super init]) {
		...
		self.timestamp = [[NSDate date] timeIntervalSince1970];
		self.userIdTo = _userId;
		self.userIdFrom = [NSString stringWithFormat:@"-%@",[SessionManager sharedInstance].userId];
		self.messageId = 0;
		self.readByUser = NO;
		self.type = blMessageDefault;
		self.isSending = NO;
...
- (BOOL)isMessageFromMe {
	return self.userIdFrom == [SessionManager sharedInstance].userId;
}
